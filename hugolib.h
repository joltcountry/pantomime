!----------------------------------------------------------------------------
!
!           Hugo Library v3.1.01.1 by Kent Tessman (c) 1995-2004
...
constant BANNER "Hugo v3.1 / Library 31011"
...
routine SpeakTo(char)
{
	...
	if char is unfriendly or char is asleep
		jump IgnorePlayer
...
routine FindObject(obj, objloc, recurse)
{
	...
	if obj = nothing or obj = player
	{
		if obj = nothing and not recurse
		{
			...
			parser_data[PARSE_RANK_TESTS] = 0
			parser_data[FINDOBJECT_KNOWN] = 0
		}
		elseif obj = player
		{
